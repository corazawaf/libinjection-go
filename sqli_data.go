...
func parseOperator2(s *sqliState) int {
	...
	ch := s.lookup(lookupOperator, []byte(s.input[s.pos:]), 2)
	if ch != byteNull {
...
func parseString(s *sqliState) int {
	// assert s.input[s.pos] == single or double quote
...
func parseMoney(s *sqliState) int {
	...
	length := indexOfLargestStr(s.input[s.pos+1:], s.length-s.pos-1, "0123456789.,")
	if length == 0 {
		if s.input[s.pos+1] == '$' {
			...
			if index == -1 {
				...
				s.current.strOpen = '$'
				s.current.strClose = '$'
				return index + s.pos + 4
			}
		} else {
			// ok it's not a number or '$$', but maybe it's pgsql "$ quoted strings"
			xlen := indexOfLargestStr(s.input[s.pos+1:], s.length-s.pos-1, "abcdefghjiklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
			if xlen == 0 {
			...
			index := strings.Index(s.input[xlen+2:s.length-s.pos], s.input[s.pos:s.pos+xlen+2])
			if index == -1 || index+xlen+2 < s.pos+xlen+2 {
				// fell off edge
				...
				// get one
				s.current.assign(sqliTokenTypeString, s.pos+xlen+2, index-s.pos, s.input[s.pos+xlen+2:])
				s.current.strOpen = '$'
				s.current.strClose = '$'
				return index + xlen + 2
			}
		}
	} else if length == 0 && s.input[s.pos+1] == '.' {
		return parseWord(s)
	} else {
		s.current.assign(sqliTokenTypeNumber, s.pos, 1+length, s.input[s.pos:])
		return s.pos + 1 + length
	}
}

func parseWord(s *sqliState) int {
	length := indexOfLargestStr(s.input[s.pos:], s.length-s.pos, " []{}<>:\\?=@!#~+-*/&|^%(),';\t\n\v\f\r\"\240\000")
	...
	if length < tokenSize {
		ch := s.lookup(lookupWord, s.current.val[:], length)
		if ch == byteNull {
...
func parseOther(s *sqliState) int {
	s.current.assignByte(sqliTokenTypeUnion, s.pos, 1, s.input[s.pos])
	return s.pos + 1
}
...
func parseEolComment(s *sqliState) int {
	...
	if len(endPos) == 0 {
		...
		s.current.assign(sqliTokenTypeComment, s.pos, s.length-len(endPos)-s.pos, s.input[s.pos:])
		return s.length - len(endPos) + 1
	}
}
...
func parseNumber(s *sqliState) int {
	...
	if s.input[s.pos] == '0' && s.pos+1 < s.length {
		...
		if digits != "" {
			length := indexOfLargestStr(s.input[s.pos+2:], s.length-s.pos-2, digits)
			if length == 0 {
	...
	pos := s.pos
	for pos < s.length && unicode.IsDigit(rune(s.input[pos])) {
		pos += 1
	}
	...
	if haveE == 1 && haveExp == 0 {
		...
		s.current.assign(sqliTokenTypeBareWord, s.pos, pos-s.pos, s.input[s.pos:])
	} else {
		s.current.assign(sqliTokenTypeNumber, s.pos, pos-s.pos, s.input[s.pos:])
	}
	return pos
}
...
func parseSlash(s *sqliState) int {
	...
	if strings.Index(s.input[s.pos+2:s.pos+index+3], "/*") != -1 ||
		isMysqlComment(s.input, s.pos) {
...
func parseTick(s *sqliState) int {
	...
	ch := s.lookup(lookupWord, s.current.val[:], s.current.len)
...
func parseVar(s *sqliState) int {
	...
	length := indexOfLargestStr(s.input[pos:], s.length-pos, " <>:\\?=@!#~+-*/&|^%(),';\t\n\v\f\r'`\"")
	if length == 0 {
...
func parseBString(s *sqliState) int {
	...
	length := indexOfLargestStr(s.input[s.pos+2:], s.length-s.pos-2, "01")
...
func parseXString(s *sqliState) int {
	...
	length := indexOfLargestStr(s.input[s.pos+2:], s.length-s.pos-2, "0123456789abcdefABCDEF")
	if s.pos+2+length >= s.length || s.input[s.pos+2+length] != byteSingle {
		return parseWord(s)
	}
...
var sqlKeywords = map[string]byte{
	"!!":                            'o',
	"!<":                            'o',
	"!=":                            'o',
	"!>":                            'o',
